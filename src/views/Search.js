import React from 'react';
import './Search.css';
import axios from 'axios';
import Recommend from '../components/Recommend';


class Search extends React.Component {

  state = {
    data: [],
    input: ''
  }

  componentDidMount() {
    this.getPopular();
  }

  getPopular() {
    axios.get("https://api.themoviedb.org/3/movie/popular?api_key=8011d764f63852f7bf4dda520102d6c9&language=en-US&page=1")
      .then((result)=>{
        this.setState({
          data: result.data.results
        })
      });
  };

  handleInputChange = (event) => {
    let value = event.target.value
    this.setState({
      input: value
    }); 
  };


  render() {
    const findMovie = this.state.data.filter((el)=>el.original_title.toLowerCase().includes(this.state.input.toLowerCase()));

    return (
      <div className="search-bar">
        <input id="search" type="text" value={this.state.input} placeholder="Search" onChange={this.handleInputChange}/>
        <img id="search-img" src="data:image/png;base64," alt="search"/>
        <ul>{findMovie.map((el)=><li key={el.id}><img src={`https://image.tmdb.org/t/p/w300/${el.poster_path}`}/></li>)}</ul>
        <Recommend name="Recommendations" data={this.state.data}/>
      </div>
    );
  };
};

export default Search;